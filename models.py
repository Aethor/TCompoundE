...
class TKBCModel(nn.Module, ABC):
    ...
    def get_ranking(
        self, queries, filters, year2id={}, batch_size: int = 1000, chunk_size: int = -1
    ):
        """
        ...
        with torch.no_grad():
            ...
            while c_begin < self.sizes[2]:
                ...
                while b_begin < len(queries):
                    if (
                        queries.shape[1] > 4
                    ):  # time intervals exist   对五元组中时间戳的处理
                        ...
                        for triple in these_queries:
                            ...
                            for key, time_idx in sorted(
                                year2id.items(), key=lambda x: x[1]
                            ):  # 时间戳转换成id
                        ...
                        start_queries = torch.from_numpy(
                            np.array(start_queries).astype("int64")
                        ).cuda()
                        end_queries = torch.from_numpy(
                            np.array(end_queries).astype("int64")
                        ).cuda()
                    ...
                    for i, query in enumerate(these_queries):
                        if queries.shape[1] > 4:
                            filter_out = filters.get(
                                int(query[0]), int(query[1]), query[3], query[4], []
                            )
...
class TCompoundE(TKBCModel):
    ...
    def get_rhs(self, chunk_begin: int, chunk_size: int):
        return (
            self.embeddings[0]
            .weight.data[chunk_begin : chunk_begin + chunk_size][:, : self.rank]
            .transpose(0, 1)
        )
